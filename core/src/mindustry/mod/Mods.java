...
public class Mods implements Loadable {
    ...
    private LoadedMod loadModNative(Fi fi, ClassLoader loader) throws Exception {
        ModMeta meta = findMetaNative(loader);
        ...
        return new LoadedMod(fi, fi, mainMod, loader, meta);
    }

    /** Tries to find the config file of a mod/plugin. */
    private @Nullable ModMeta findMetaNative(ClassLoader loader){
        InputStream metaFile = null;
        for(String name : metaFiles){
            metaFile = loader.getResourceAsStream(name);
            if(metaFile != null){
                break;
            }
        }
        ...
        try {
            ModMeta meta = json.fromJson(ModMeta.class, Jval.read(Streams.copyString(metaFile)).toString(Jformat.plain));
            meta.cleanup();
            return meta;
    ...
    /** Use native loader to load mods. */
    private void loadNative() {
        try {
            String modPrefixes = Streams.copyString(Objects.requireNonNull(getClass().getClassLoader().getResourceAsStream("META-INF/mods")));

            var metas = new Seq<ModMeta>();
            var mapping = new ObjectMap<String, ClassLoader>();
            var mappingFi = new ObjectMap<String, Fi>();

            for (String prefix : modPrefixes.split("\r?\n")) {
                prefix = prefix.trim();
                ...
                var loader = new ClassLoader(getClass().getClassLoader()) {
                    ...
                    public URL getResource(String name) {
                        return getParent().getResource(s + name);
                    }

                    @Override
                    public Enumeration<URL> getResources(String name) throws IOException {
                        return getParent().getResources(s + name);
                    }

                    @Override
                    public InputStream getResourceAsStream(String name) {
                        Log.info("Trying do get resource " + s + name);
                        return getParent().getResourceAsStream(s + name);
                    }
                };
                var meta = findMetaNative(loader);
                if (meta == null) throw new RuntimeException("Failed to load metadata for mod " + prefix);

                metas.add(meta);
                mapping.put(meta.internalName, loader);
                mappingFi.put(meta.internalName, new IncludedFi(loader.getParent(), prefix));
            }

            var resolved = resolveDependencies(metas);

            for (var entry : resolved) {
                var loader = mapping.get(entry.key);
                var fi = mappingFi.get(entry.key);

                Log.debug("[Mods] Loading mod @", entry.key);

                try {
                    var mod = loadModNative(fi, loader);
                    mod.state = entry.value;
                    mods.add(mod);
                    lastOrderedMods = null;

                } catch (Exception e) {
                    Log.err("An error has occured when loading mod " + entry.key, e);
                }
            }
